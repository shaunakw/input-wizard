...
#[tauri::command]
async fn start_keydown(state: State<'_, AppState>, millis: u64, key: Key) -> Result<(), ()> {
  let on = &state.on;
  if !*on.lock().await {
    *on.lock().await = true;
...
fn main() {
  tauri::Builder::default()
    .manage(AppState::new())
    .invoke_handler(tauri::generate_handler![start_click, start_keypress, start_keydown, stop])
